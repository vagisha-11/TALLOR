...
class PrecisionRecallF1:
    ...
    def __call__(
        ...
        recall: ArrayLike = None,      # (batch_size, seq_len)
        duplicate_check: bool = True,
        bucket_value: ArrayLike = None # (batch_size, seq_len)
    ):
        # Normalize inputs to CPU tensors (supports list/np/tensor) [NumPy 1.24 dtype changes]
        preds_t = to_tensor_long(predictions)     # int64 [web:221][web:252]
        labels_t = to_tensor_long(labels)         # int64 [web:221][web:252]
        ...
        if self._reduce == "micro":
            ...
            if bucket_value is not None:
                # np.bincount requires integer indices and numeric weights [web:269][web:257]
                bucket_np = (to_tensor_long(bucket_value) * mask_t).cpu().numpy().reshape(-1).astype(np.int64)
                matched_np = matched.cpu().numpy().reshape(-1).astype(np.float64)
                predicted_np = predicted.cpu().numpy().reshape(-1).astype(np.float64)
                whole_subset_np = whole_subset.cpu().numpy().reshape(-1).astype(np.float64)

                count = np.bincount(bucket_np, weights=matched_np)
                precision_arr = np.bincount(bucket_np, weights=predicted_np)
                recall_arr = np.bincount(bucket_np, weights=whole_subset_np)

                for b, v in enumerate(count):
                    self._bucket["count"][b] += float(v)
                for b, v in enumerate(precision_arr):
                    self._bucket["precision"][b] += float(v)
                for b, v in enumerate(recall_arr):
                    self._bucket["recall"][b] += float(v)

        else:
            # Kept for completeness though __init__ blocks macro
...
class DataPrecisionRecallF1:
    ...
    def __call__(
        ...
        # Normalize to CPU tensors first to avoid list/.cpu() errors
